...
public class CommandWorldPrimer extends CommandBase
{
    ...
    public CommandWorldPrimer()
    {
        ...
        this.registerSubCommand(new SubCommandReadStructure(this));
    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] args) throws CommandException
    {
        ...
        else if (cmd.equals("help"))
        {
            if (args.length == 1)
            {
                sendMessage(sender, "worldprimer.commands.help.generic.usage", this.getUsage(sender));
            }
            else if (args.length == 2)
            {
                ...
                {
                    throwCommand("worldprimer.commands.error.unknowncommandvariant", args[1]);
                }
            }
            else
            {
